...
const RideHistory = () => {
    ...
    async function fetchData() {
        try {
          ...
          const response = await userInstance.get(`/myrideinfo/${user._id}`, options);
          
          console.log(response.data, "💕💕💕💕💕");
          const currentDate = new Date(); // Get the current date and time

          // Filter the response data to retain only records with dates in the past
          const pastData = response.filter((item) => {
  ...
  {rides.map((ride, index) => (
        ...
        className="shadow-md mx-auto max-w-sm transform hover:-translate-y-1 duration-300 hover:shadow-xl cursor-pointer">
            <div className="max-h-140 overflow-hidden">
                <img className="w-full h-auto"  src={`http://localhost:5000/images/${ride.rideInfo.VehicleImage[0]}`}
                    alt="" />
            </div>
            <div className="p-7 my-auto pb-12 ">
                <h1 className="text-2xl font-semibold text-gray-700">{ride.rideInfo.date}</h1>
                <h1 className="text-2xl font-semibold text-gray-700">{ride.rideInfo.pickUp} - {ride.rideInfo.dropOff} </h1>
