...
const clickHandler=async (data)=>{
    ...
    //   };
      //setAccessToken(token)
      const chatId = await userInstance.get(`/getChatId?Id1=${data._id}&Id2=${user._id}`);
...
console.log(selectedChatId,"eee"); 
  ...
  async function fetchData() {
    try {
      ...
      const response = await userInstance.get(`/getActiveConnections/${user._id}`, options);
...
if(Payment.includes('Need Payment')){
  const amount = Payment.split('Need Payment')[1].trim().replace(/[, ]/g, "");
  const shortAmount= amount*.10
  // console.log('====================================');
  // console.log(amount);
  // console.log('====================================');
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await userInstance.post(`/create-checkout-session`, body);
        ...
        if (response.data.url) {
          ...
          const response2 = await userInstance.put(`/comformRide`,body, options);
        } else {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      const response = await userInstance.put(`/comformRide`,body, options);
      }
      Swal.fire(
        'Success!',
        'Your Ride is conformed.',
  ...
  const clickConnectionBlock = async(itemId,element) => {
   const li=element.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement
   console.log(li,"kk");
   ...
   Swal.fire({
    ...
    try {
      ...
      const response = await userInstance.put(`/blockUserbyUser?Id1=${itemId}&Id2=${user._id}`, options);
      li.remove()
 ...
 const toggleDropdown = (index) => {
  ...
  return (
    ...
    <div className='grid grid-cols-6' style={{minHeight:'660px'}}>
        <div className='col-span-2 relative px-4 py-4'>
 ...
 {connections.map((item,index) => (
   ...
   className={`mb-5 pt-3 px-5 py-3 sm:py-4 rounded-full flex items-center justify-between ${
     item.rideInfo.user !== item.userDetails._id ? 'bg-emerald-800' : 'bg-zinc-500'
   }`}
 >
     ...
     <HiChatBubbleLeftRight onClick={() =>{clickHandler(item.userDetails)} } />
        ...
        >
          ...
          {item.rideInfo.user == item.userDetails._id ? (
            <li>
              <a
                href="#"
                onClick={() => ComformOption(item.connectionId, item.rideInfo)}
            ...
            <li>
              <a
                href="#"
                onClick={(e) => clickConnectionBlock(item.userDetails._id,e.target)}
                className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
              >
                UnFollow
              </a>
            ...
            <li>
              <a
                href="#"
                onClick={() => OpenClickOption(item.userDetails._id)}
                className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
              >
                Review
              </a>
    ...
    <form onSubmit={(e) => handleSubmit(e, item.userDetails._id)}>
